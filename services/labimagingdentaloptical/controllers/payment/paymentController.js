...
const handlePaymentIntentSucceeded = async (data) => {
    console.log("PLAN METADATA==>", data.metadata);
    ...
    if(data.metadata.type === 'subscription'){
        ...
        const content = SubscriptionMail(userData?.email, userData?.full_name,"Subscription Plan Purchased", data?.metadata?.plan_price);
...
class PaymentController {
    async getPaymentHistory(req, res) {
        try {
            ...
            if (order_type !== undefined) {
                ...
                } else if (order_type == "test_order") {
                    myFilter.order_type = "test_order"
                }
            }
            ...
            const query = [
...

                {
                    $lookup: {
                        from: "portalusers",
                        let: { forUserId: { $toObjectId: "$for_user" } }, // Convert for_user to ObjectId
                        pipeline: [
                            {
                                $match: {
                                    $expr: {
                                        $eq: ["$_id", "$$forUserId"], // Compare _id with converted ObjectId
                                    },
                                },
                            },
                ...
                { $unwind: "$portalusersData" },
                {
                    $lookup: {
                        from: "basicinfos",
                        let: { forId: { $toObjectId: "$for_user" } }, // Convert for_user to ObjectId
                        pipeline: [
                            {
                                $match: {
                                    $expr: {
                                        $eq: ["$for_portal_user", "$$forId"], // Compare _id with converted ObjectId
                                    },
                                },
                            },
...

                {
                    $project: {
                        ...
                        Name: { $concat: ["$profileinfoData.first_name", " ", "$profileinfoData.last_name"] }
    ...
    async subscriptionPurchasedPlans(req, res) {
        try {
            const { page, limit, user_id, type } = req.query
    ...
    async createPaymentIntent(req, res) {
        try {
            ...
            let portal_type = req.body.portal_type;

            if (req.body.order_type === 'test_order') {
    ...
    async isPlanPurchased(req, res) {
        try {
            const { user_id, portal_type } = req.query;
            console.log("isPlanPurchasedisPlanPurchased", req.query);
            var isPlanPurchased = false
            const currentDate = new Date();
            currentDate.setHours(0, 0, 0, 0);

            const query = { for_user: user_id, expiry_date: { $gte: currentDate }, portal_type };
    ...
    async getPaymentHistory_fourportal(req, res) {
        try {
            ...
            const query = [
...

                {
                    $project: {
                        ...
                        paymentBy: "$portalusersData.role",
                        Name: { $concat: ["$basicinfosData.first_name", " ", "$basicinfosData.last_name"] }
