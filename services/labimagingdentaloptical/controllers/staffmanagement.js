...
class StaffController {
    async addStaff(req, res) {
      ...
      last_name,
        ...
        try {
            // const staffCount = await PortalUser.countDocuments({created_by_user:creatorId,isDeleted:false,type:type,role:"STAFF"});
            ...
            if (checkUser?.role === 'STAFF') {
                staffCount = await PortalUser.countDocuments({ role:"STAFF", created_by_user: checkUser?.created_by_user, isDeleted: false });
                // console.log("staffCount>>>>>>>>>",staffCount)
                checkPlan = await SubscriptionPurchaseStatus.find({ for_user: checkUser?.created_by_user });
            } else {
                staffCount = await PortalUser.countDocuments({role:"STAFF", created_by_user: creatorId, isDeleted: false });
            ...
            if (checkCondition?.statusData === "active") {
                ...
                if(shouldAddStaff){
                    ...
                    if (profilePic != null && profilePic != "") {
                        profilePicData = await DocumentInfo.findOne({ _id: profilePic });
                        // console.log("profileData==>>>>>>",profilePicData?.url)
                    }
                    ...
                    let userData = new PortalUser(
                        {
                            ...
                            profile_picture: profilePicData?.url,
                    ...
                    //     },
                    // })
                    const content = sendStaffDetails(email, password, type);
    ...
    async editStaff(req, res) {
        ...
        try {
            ...
            console.log("profilePicData?.url>>>>>>>",profilePicData?.url)
    ...
    async getStaffDetails(req, res) {
        try {
            ...
            console.log(req.query,"check query check");
            let staffFullDetails = await StaffInfo.findOne({ for_portal_user: staffId, type })
                .populate({
                    path: "in_profile",
                    populate: {
          ...
          console.log("specialityData-----",specialityData.data[0]?._id);
            ...
            if (staffFullDetails && staffFullDetails.profile_picture) {
                // let data1 = await DocumentInfo.findOne({_id: mongoose.Types.ObjectId(staffFullDetails.profile_picture)});
                // // rest of the code
                // if(data1){
                    staffFullDetails.in_profile.profile_picture = await getDocument(staffFullDetails.profile_picture);
                // }
            }
    ...
    async getAllStaff(req, res) {
        try {
            ...
            //     staffFullDetails[0].paginatedResults[index].StaffProfiles.profilePictureSignedUrl = ""
            //     const profilePicKey = staffFullDetails[0].paginatedResults[index].StaffProfiles.profile_picture;
            //     if (profilePicKey != "") {
            ...
            //         staffFullDetails[0].paginatedResults[index].StaffProfiles.profilePictureSignedUrl = pharmacyLogo.data[0]
            //     }
            // }
