...
export const getNotification = async (req, res) => {
  try {
    ...
    //   'Authorization': req.headers['authorization']
    // }
    var {page,limit,for_portal_user} = req.query;

    let checkUser = await PortalUser.findOne({_id:mongoose.Types.ObjectId(for_portal_user)})

    if(checkUser.role === "INSURANCE_STAFF"){
        let staffData = await StaffInfo.findOne({for_portal_user : mongoose.Types.ObjectId(for_portal_user)})
        for_portal_user = staffData?.for_user; 
    }

    console.log("for_portal_user-----------",for_portal_user);
    ...
    //       };
    //       if (element.created_by_type == 'pharmacy') {
    //         let ids = element.created_by;
    //         let resData = await httpService.getStaging('pharmacy/get-portaluser-data', { data: ids }, headers, 'pharmacyServiceUrl');
    //         console.log("res>>>>>",resData)
    //         object.name = resData?.data[0]?.user_name
